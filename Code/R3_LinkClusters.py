...
if Mode=='R':
   ...
   if UserAnswer=='Y':
      ...
      UF.RecCleanUp(AFS_DIR, EOS_DIR, 'R3', ['R3_R3','R3_R4'], "SoftUsed == \"EDER-GNN-R3\"")
      ...
      for k in range(0,Zsteps):
        for i in range(0,Xsteps):
            ...
            ScriptName = AFS_DIR + '/Code/Utilities/R3_LinkClusters_Sub.py '
            UF.SubmitJobs2Condor([OptionHeader, OptionLine, SHName, SUBName, MSGName, ScriptName, Ysteps, 'EDER-GNN-R3', False,False])
   ...
   for k in range(0,Zsteps):
       ...
       for i in range(0,Xsteps):
           for j in range(0,Ysteps):
            ...
            MSGName = AFS_DIR + '/HTCondor/MSG/MSG_R3_'+ str(k) + '_'+ str(i)+ '_'+ str(j)
            ScriptName = AFS_DIR + '/Code/Utilities/R3_LinkClusters_Sub.py '
            if os.path.isfile(required_output_file_location)!=True:
               bad_pop.append([OptionHeader, OptionLine, SHName, SUBName, MSGName, ScriptName, 1, 'EDER-GNN-R3', False,False])
       ...
       if args.Log!='Y':
           print(UF.TimeStamp(), bcolors.OKGREEN+"Cluster generation is completed, you can start applying GNN on them now"+bcolors.ENDC)
                    ...
                    if os.path.isfile(required_output_file_location)!=True and args.Force=='N':
                       ...
                       exit()
                        ...
                        for cd in cluster_data:
                            result_temp=cd.RecStats
                            recall_temp=cd.Stats
                            ...
                            fake_results_7.append(int(result_temp[1][5]))
                            ...
                            truth_results_7.append(int(result_temp[2][5]))
                            try:
                                ...
                                precision_results_7.append(int(result_temp[2][5])/(int(result_temp[2][5])+int(result_temp[1][5])))
                                ...
                                recall_results_7.append(int(result_temp[2][5])/(int(recall_temp[2][2])))
                            except:
                               continue
                            label=result_temp[0]
            ...
            print(tabulate([[label[0], np.average(fake_results_1), np.average(truth_results_1), np.average(precision_results_1), np.std(precision_results_1), np.average(recall_results_1), np.std(recall_results_1)], \
                            ...
                            [label[6], np.average(fake_results_7), np.average(truth_results_7), np.average(precision_results_7), np.std(precision_results_7), np.average(recall_results_7), np.std(recall_results_7)]], \
